...
public class decision<T extends Comparable<T>> {
    Node base;
    ...
    public void print() {
        ...
        int nodeCount = nodeCount(base, 0);
        int width = 50;
        ...
        list.get(height / 2).set(0, new PrintNode(base));
    ...
    public int maxDepth() {
        return maxDepthNew(0, base);
    }

    private int maxDepthNew(int depth, Node node) {
        depth++;
        int left = depth;
        int right = depth;
        if (node.left != null)
            left = maxDepthNew(depth, node.left);
        if (node.right != null)
            right = maxDepthNew(depth, node.right);
